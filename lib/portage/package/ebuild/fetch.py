...
class FilenameHashLayout:
	...
	def _get_fnhash(self, filename):
		return checksum_str(filename.encode('utf8'), self.algo)

	def get_path(self, filename):
		fnhash = self._get_fnhash(filename)
		ret = ''
...
class ContentHashLayout(FilenameHashLayout):
	...
	def _get_fnhash(self, filename):
		return filename.digests[self.algo]

	def get_path(self, filename):
		fnhash = remaining = self._get_fnhash(filename)
		ret = ''
		for c in self.cutoffs:
			assert c % 4 == 0
			c = c // 4
	...
	def verify_args(args, filename=None):
		...
		algorithms are constrained by the filename digests attribute.
